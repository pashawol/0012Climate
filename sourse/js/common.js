...
var JSCCommon = {
    ...
    CustomYoutubeBlock: function() {
        $(".pretty-embed__bg").each(function() {
            ...
            $(this).click(function() {
                $(this).removeClass("on").next().attr("src", 'https://www.youtube.com/embed/' + $(this).data("src") + '?autoplay=1').addClass("on");
            })
        })
    },
    inputMask: function() {
...
jQuery(document).ready(function($) {
    ...
    function sliderMobile(){
        ...
        // слайдер в модалке
        $('.modalSlider').click(function() {
            var href = $(this).attr('href');
            $('.modal-object__slider-small, .modal-object__slider-big', $(href)).slick('refresh');
        })

        $('.s-sert__slider--js,' + ' .modal-object__slider-big , ' + ' .modal-object__slider-small  ').on('lazyLoaded', function(event, slick, image, imageSource) {
    ...
    $('.objectsLink').click(function(event) {
        ...
        if (slug !== href) {
            ...
            $objectsItems.html("");
            $('.modal-object').remove();
            $('#objectsContainer').addClass('loaded');
            $.ajax({
                url: phpvar.url,
                type: 'POST',
                dataType: 'json',
                ...
                $objectsItems.html(response.html);
                if (response.modals !== "") {
                ...
                */
                ...
                $('#objectsContainer').removeClass('loaded');
            });
        }
    });
...
jQuery(document).ready(function($) {
       $('.cookie_pr').click(function(event) {
           event.preventDefault();
           Cookies.set('cookie_pr', 'value', { expires: 31 });
           $('.cookie_block_over').hide();
       });
       $('.cookie_not').click(function(event) {
           event.preventDefault();
           $('.cookie_block_over').hide();
       });
});